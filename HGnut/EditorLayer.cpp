...
namespace hg {
	...
	{
		HG_PROFILE_FUNCTION();
		m_CheckerboardTexture = Texture2D::Create("assets/textures/1.jpg");
	...
	{ 
		...
		if (path.extension().string() != ".hg")
		{
			HG_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	void EditorLayer::OnSceneStop()
	{
		...
		HG_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
	...
	void EditorLayer::UI_Toolbar()
	{
		...
		ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));		// 默认颜色
		auto& colors = ImGui::GetStyle().Colors;
		const auto& buttonHovered = colors[ImGuiCol_ButtonHovered];		// 悬停变色
		ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(buttonHovered.x, buttonHovered.y, buttonHovered.z, 0.5f));
		const auto& buttonActive = colors[ImGuiCol_ButtonActive];		// 点击变色
