...
#include "hg/Scene/SceneSerializer.h"
...
namespace hg {
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		auto viewportOffest = ImGui::GetWindowPos();

		m_ViewportBounds[0] = { viewportMinRegion.x + viewportOffest.x, viewportMinRegion.y + viewportOffest .y};
		m_ViewportBounds[1] = { viewportMaxRegion.x + viewportOffest.x, viewportMaxRegion.y + viewportOffest.y };

		m_ViewportFocuse = ImGui::IsWindowFocused(); // 窗口聚焦
		m_ViewportHovered = ImGui::IsWindowHovered(); // 窗口悬停
		Application::Get().GetImGuiLayer()->SetBlockEvents(!m_ViewportFocuse && !m_ViewportHovered);
		...
		if (selectedEntity && m_GizmoType != -1)
		{
			...
			ImGuizmo::SetRect(ImGui::GetWindowPos().x, ImGui::GetWindowPos().y, windowWidth, windowHeight);	// 设置 指针 方向

			// Camera
			//auto cameraEntity = m_ActiveScene->GetPrimaryCameraEntity();
			//const auto& camera = cameraEntity.GetComponent<CameraComponent>().Camera;
			//const glm::mat4& cameraProjection = camera.GetProjection();
			//glm::mat4 cameraView = glm::inverse(cameraEntity.GetComponent<TransformComponent>().GetTransform()); // 视觉矩阵是相机矩阵的逆
	...
	void EditorLayer::OnEvent(hg::Event& e)
	{
	...
	{
		std::string filepath = FileDialogs::SaveFile("hg Scene (*.hg)\0*.hg\0");
		if (!filepath.empty())
		{
			SceneSerializer serializer(m_ActiveScene);
